...
echo ""
sudo apt-get update 1>/dev/null 2>> ./install.log && sudo apt-get upgrade -y 1>/dev/null 2>> ./install.log && sudo apt-get update 1>/dev/null 2>> ./install.log && sudo apt-get install amass sherlock forensics-extra veil bloodhound libboost-all-dev cmake pwncat routersploit sqlmap armitage npm nodejs g++ libssl-dev cewl libgmp3-dev libmpc-dev qpdf unrar sed xxd libc-bin curl jq perl gawk grep coreutils git golang seclists enum4linux feroxbuster nbtscan nikto nmap onesixtyone oscanner smbclient smbmap smtp-user-enum sslscan tnscmd10g whatweb fcrackzip hashcat hash-identifier python3-pip -y 1>/dev/null 2>> ./install.log
...
echo ""
git clone https://github.com/digininja/twofi.git $crypto/twofi/ 1>/dev/null 2>> ./install.log
...
echo ""
git clone https://github.com/EgeBalci/HERCULES.git $exploit/HERCULES/ 1>/dev/null 2>> ./install.log
cd $exploit/HERCULES 1>/dev/null 2>> ./install.log
go get github.com/fatih/color 1>/dev/null 2>> ./install.log
go run Setup.go 1>/dev/null 2>> ./install.log
...
echo ""
git clone https://github.com/java-decompiler/jd-gui.git $revbin/jd-gui/ 1>/dev/null 2>> ./install.log
cd $revbin/jd-gui 1>/dev/null 2>> ./install.log
./gradlew build 1>/dev/null 2>> ./install.log
ln -s $revbin/jd-gui ~/bin/jd-gui 1>/dev/null 2>> ./install.log
